...
def get_model_var_type(model_var_type_name):
    if model_var_type_name == 'learned_sigma':
        return ModelVarType.LEARNED_RANGE
    elif model_var_type_name == 'fixed_small':
        return ModelVarType.FIXED_SMALL
    elif model_var_type_name == 'fixed_large':
        return ModelVarType.FIXED_LARGE
    else:
        raise ValueError('only model_var_type_name supported: learned_sigma, fixed_small, fixed_large.')
    pass

def get_loss_type(loss_type_name):
    if loss_type_name == 'kl':
        return LossType.RESCALED_KL
    elif loss_type_name == 'rescaled_mse':
...
class BaseDiffusion:
    ...
    def q_sample(self, x_start, t, x_T_end=None):
        ...
        :param x_T_end: if specified, the split-out normal noise.
    ...
    def _scale_timesteps(self, t):
        if self.rescale_timesteps:
            return t.float() * (self.timesteps_scaling_factor / self.num_timesteps)
        return t
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            device=device,
            progress=progress,
            diffusion_start_point=diffusion_start_point,
        ):
            final = sample
        return final["sample"]
    ...
    def p_sample_loop_progressive(
        ...
        start_point = self.num_timesteps # start from time T
    ...
    def ddim_sample(*k, **kw):
        raise NotImplementedError

    def ddim_reverse_sample(*k, **kw):
        raise NotImplementedError

    def ddim_sample_loop(*k, **kw):
        raise NotImplementedError

    def ddim_sample_loop_progressive(*k, **kw):
        raise NotImplementedError
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None):
        ...
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param x_T_end: if specified, the specific Gaussian noise to try to remove.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
                B, C = x_t.shape[:2]
                assert model_output.shape == (B, C * 2, *x_t.shape[2:]), f'output: {model_output.shape}, expected: ({B},{C * 2},{x_t.shape[2:]})'
                model_output, model_var_values = torch.split(model_output, C, dim=1)
