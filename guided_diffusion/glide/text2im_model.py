...
class UnetConditionModel(UNetModel):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.xf_width:
            ...
            #self.cond_projection = nn.Linear(xf_width, self.model_channels * 4)
            self.cond_projection = FullyConnected(xf_width, self.model_channels * 4, layers_num=2, act_last=False)


    def convert_to_fp16(self):
        super().convert_to_fp16()
        if self.xf_width:
            self.cond_projection.to(torch.float16)
        for i in self.children():
            if hasattr(i, 'convert_to_fp16') and callable(i.convert_to_fp16):
                i.convert_to_fp16()



    def forward(self, x, timesteps=None, clip_condition=None, **kwargs):
        hs = []
        if timesteps is None:
            timesteps = torch.ones(x.shape[0], 1, device=x.device)
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        if self.xf_width:
            # text_outputs = self.get_text_emb(tokens, mask)
            # print(clip_condition.dtype, self.dtype)
            xf_proj, xf_out = self.cond_projection(clip_condition.type(self.dtype)), None #text_outputs["xf_proj"], text_outputs["xf_out"]
